...
if SERVER then
    ...
    function SWEP:Error(type, errorEnt)
        ...
        timer.Create(self.defiTimer, GetConVar("ttt_mtr_defibrillator_error_time"):GetFloat(), 1, function()
    ...
    function SWEP:BeginRevival(ragdoll, bone)
        ...
        local reviveTime = GetConVar("ttt_mtr_defibrillator_revive_time"):GetFloat()
        local doResetConfirmation = GetConVar("ttt_mtr_defibrillator_reset_confirm"):GetBool()
        local reviveHealth = GetConVar("ttt_mtr_defibrillator_revival_health"):GetInt()
    ...
    function SWEP:FinishRevival(ply, owner)
        ...
        if math.random(0, 100) > GetConVar("ttt_mtr_defibrillator_success_chance"):GetInt() then
    ...
    function SWEP:PlaySound(soundName)
        if not GetConVar("ttt_mtr_defibrillator_play_sounds"):GetBool() then
            return
        end
    ...
    function SWEP:Think()
        ...
        if CurTime() >= self:GetStartTime() + GetConVar("ttt_mtr_defibrillator_success_chance"):GetInt() - 0.01 then
    ...
    function SWEP:PrimaryAttack()
        ...
        if CORPSE.WasHeadshot(ent) and not GetConVar("ttt_mtr_defibrillator_revive_braindead"):GetBool() then
